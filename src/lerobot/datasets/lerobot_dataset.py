...
class LeRobotDatasetMetadata:
    ...
    def commit_episode_counters(self, episode_length: int) -> None:
        """Bump in-memory totals so subsequent episodes see correct indices.

        This separates the cheap counter update (must be visible to the next episode immediately) from the
        on-disk write performed inside ``save_episode``. Async recording paths invoke this on the main thread
        and let a worker call ``save_episode(..., update_counters=False)`` to flush metadata to disk.
        """
    ...
    def save_episode(
        ...
        *,
        ...
        if update_counters:
            self.commit_episode_counters(episode_length)
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Async video encoding state (opt-in via start_async_encoder()).
        ...
        self._encode_error_lock = threading.Lock()
        self._async_defer_stats = False
        self._async_defer_save_episode_data = False
        self._async_next_episode_index: int | None = None
    ...
    def finalize(self):
        ...
        """
        # Make sure any pending async video encoding finishes before flushing parquet writers,
        # otherwise meta/episodes parquet rows for those episodes would be lost.
        self.drain_async_encoder(raise_errors=False)
        self.stop_async_encoder(raise_errors=False)
        self._close_writer()
    ...
    @staticmethod
    def _clone_episode_buffer_for_async(episode_buffer: dict) -> dict:
        """Create a detached copy safe to consume after main-thread buffer reset."""
        cloned: dict = {}
    ...
    def save_episode(
        ...
        When ``start_async_encoder()`` has been called beforehand (and ``batch_encoding_size == 1``), the heavy
        video-encoding work and the per-episode metadata write are handed off to a single background worker so
        that the recording loop can immediately move on to the next episode (keeping teleop responsive).
        ...
        ep_stats = None if (self._async_video_encoding and self._async_defer_stats) else compute_episode_stats(
            episode_buffer, self.features
        )

        ep_metadata = (
            {}
            if (self._async_video_encoding and self._async_defer_save_episode_data)
            else self._save_episode_data(episode_buffer)
        )
        ...
        if self._async_video_encoding:
            if use_batched_encoding:
                raise ValueError(
                    "Async video encoding is incompatible with batch_encoding_size > 1. "
                    "Stop the async encoder or set batch_encoding_size=1."
                )
            self._raise_if_async_encoder_failed()
            if self._async_next_episode_index is not None and episode_index != self._async_next_episode_index:
                raise RuntimeError(
                    "Async episode ordering mismatch: "
                    f"expected {self._async_next_episode_index}, got {episode_index}."
                )
            if self._async_next_episode_index is not None:
                self._async_next_episode_index += 1
            episode_buffer_snapshot = None
            if ep_stats is None or self._async_defer_save_episode_data:
                # The episode buffer will be reset right after enqueue; keep a detached snapshot for worker steps.
                episode_buffer_snapshot = self._clone_episode_buffer_for_async(episode_buffer)
            episode_buffer_for_stats = episode_buffer_snapshot if ep_stats is None else None
            episode_buffer_for_save = (
                episode_buffer_snapshot if self._async_defer_save_episode_data else None
            )
            self.meta.commit_episode_counters(episode_length)
            self._encode_queue.put(
                {
                    ...
                    "episode_buffer_for_stats": episode_buffer_for_stats,
                    "episode_buffer_for_save": episode_buffer_for_save,
    ...
    def start_async_encoder(
        self, *, defer_stats: bool = False, defer_save_episode_data: bool = False
    ) -> None:
        """Start a background worker that handles video encoding + episode metadata writes serially.

        After this call, ``save_episode`` only writes parquet frame data and bumps in-memory counters on the
        main thread; the worker performs the heavy CPU work for previous episodes concurrently with new
        teleop recording. Must be paired with ``finalize()`` (or letting ``VideoEncodingManager`` exit) to
        ensure all queued tasks are drained and any errors are surfaced.
        """
        if self._encode_thread is not None:
            return
        if self.batch_encoding_size != 1:
            raise ValueError(
                "Async video encoding requires batch_encoding_size=1; "
                f"got {self.batch_encoding_size}."
            )
        self._async_video_encoding = True
        self._async_defer_stats = defer_stats
        self._async_defer_save_episode_data = defer_save_episode_data
        self._async_next_episode_index = self.meta.total_episodes
        self._encode_queue = queue.Queue()
        with self._encode_error_lock:
            self._encode_error = None
        self._encode_thread = threading.Thread(
            target=self._async_encode_loop, name="LeRobotAsyncEncoder", daemon=True
        )
        self._encode_thread.start()
        logging.info(
            "LeRobotDataset async video encoder started (defer_stats=%s, defer_save_episode_data=%s).",
            defer_stats,
            defer_save_episode_data,
        )

    def _raise_if_async_encoder_failed(self) -> None:
        with self._encode_error_lock:
            err = self._encode_error
            self._encode_error = None
        if err is not None:
            raise err

    def _async_encode_loop(self) -> None:
        while True:
            ...
            try:
                ...
                try:
                    self._finalize_episode_async(**item)
                    elapsed_s = time.perf_counter() - start_ts
                    pending = self._encode_queue.qsize()
                    logging.info(
                        "Async episode %s video+meta done in %.2fs (queue_pending=%s).",
                        episode_index,
                        elapsed_s,
                        pending,
                    )
                except BaseException as exc:  # noqa: BLE001 - propagate to drain caller
                    with self._encode_error_lock:
                        if self._encode_error is None:
                            self._encode_error = exc
                    logging.exception(
                        "Async video encoding failed for episode %s", item.get("episode_index")
                    )
            finally:
                self._encode_queue.task_done()

    def _finalize_episode_async(
        self,
        episode_index: int,
        episode_length: int,
        episode_tasks: list[str],
        episode_stats: dict[str, dict] | None,
        episode_buffer_for_stats: dict | None,
        episode_buffer_for_save: dict | None,
        episode_metadata: dict,
        parallel_encoding: bool,
    ) -> None:
        ep_metadata = dict(episode_metadata)
        if episode_buffer_for_save is not None:
            ep_metadata.update(self._save_episode_data(episode_buffer_for_save))

        if episode_stats is None:
            stats_source = episode_buffer_for_stats if episode_buffer_for_stats is not None else episode_buffer_for_save
            if stats_source is None:
                raise RuntimeError(
                    "Async deferred stats enabled but episode_buffer_for_stats was not provided."
                )
            episode_stats = compute_episode_stats(stats_source, self.features)
        if len(self.meta.video_keys) > 0:
            num_cameras = len(self.meta.video_keys)
            if parallel_encoding and num_cameras > 1:
                with concurrent.futures.ProcessPoolExecutor(max_workers=num_cameras) as executor:
    ...
    def drain_async_encoder(self, *, raise_errors: bool = True) -> None:
        """Block until all queued encoding tasks finished, optionally re-raising the first worker error."""
    ...
    def stop_async_encoder(self, *, raise_errors: bool = True) -> None:
        """Drain, then signal the worker to exit. Safe to call multiple times."""
        if self._encode_thread is None:
            return
        ...
        self._async_defer_stats = False
        self._async_defer_save_episode_data = False
        self._async_next_episode_index = None
        if raise_errors:
            self._raise_if_async_encoder_failed()
    ...
    def create(
        ...
        obj._encode_queue = None
        obj._encode_thread = None
        obj._encode_error = None
        obj._encode_error_lock = threading.Lock()
        obj._async_defer_stats = False
        obj._async_defer_save_episode_data = False
        obj._async_next_episode_index = None
