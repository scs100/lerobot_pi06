...
class VideoEncodingManager:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        # Drain async video encoder first so all queued episodes are committed before any cleanup. We avoid
        # raising here to preserve the original exception (if any); finalize() will surface it via logs and
        # downstream callers can re-check via drain_async_encoder().
        if hasattr(self.dataset, "drain_async_encoder"):
            try:
                self.dataset.drain_async_encoder(raise_errors=(exc_type is None))
            except Exception:
                logging.exception("Async video encoder reported errors during drain.")
            try:
                self.dataset.stop_async_encoder(raise_errors=False)
            except Exception:
                logging.exception("Failed to stop async video encoder cleanly.")
