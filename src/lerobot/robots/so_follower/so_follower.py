...
def _auto_use_saved_calibration() -> bool:
    return os.getenv("LEROBOT_AUTO_USE_CALIBRATION", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
...
class SOFollower(Robot):
    ...
    def calibrate(self) -> None:
        if self.calibration:
            if _auto_use_saved_calibration():
                logger.info(
                    f"Auto-using calibration file associated with the id {self.id} and writing it to motors"
                )
